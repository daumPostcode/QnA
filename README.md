...
1. **서제스트 옵션 추가**
    ...
    검색어 입력시 작동하는 suggest 목록의 최대 노출 갯수를 조절할 수 있는 생성자 옵션을 추가하였습니다. (변수명: maxSuggestItems)
    해당 기능은 우편번호 서비스 사용시 높이를 작게 설정하여 이용하시는 경우, 스크롤 문제가 자주 발생 할 수 있는데 높이에 맞춰서 suggest의 최대 갯수를 설정하여 이용하시면 됩니다. 자세한 사항은 가이드 페이지의 속성 부분을 참고해 주시기 바랍니다.
