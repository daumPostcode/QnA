...
### 2019. 9. 5
1. javascript 로딩 URL이 변경되었습니다. 기존의 정적로딩/동적로딩을 구분하던 스크립트에서, 통합 스크립트로 변경되었습니다. 사용방식은 기존과 다르지 않기 때문에, 단순하게 URL만 변경하시면 기존 서비스에서 그대로 사용가능 합니다. 자세한 사항은 [공식가이드페이지](http://postcode.map.daum.net/guide)를 참고 부탁드립니다.
